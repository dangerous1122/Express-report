            ...
            <dl className="grid max-w-xl grid-cols-1 gap-x-8 gap-y-10 lg:max-w-none lg:grid-cols-2 lg:gap-y-16">
              {features.map((feature) => (
                <div key={feature.name} className="relative pl-16">
                  <dt className="font-semibold leading-7 text-gray-900 text-2xl">
                    <div
                      className="absolute left-3 top-2 flex h-5 w-5 items-center justify-center rounded-full "
                      style={{ background: "#ca0a66" }}
                    ></div>
                    {feature.name}
                  </dt>
                  {feature.description.map((desc) => (
                    <dd className="mt-2 text-lg leading-7 text-gray-600">
                     <span className="font-bold text-xl">{'>'}</span> {desc}
                    </dd>
                  ))}
                </div>
              ))}
